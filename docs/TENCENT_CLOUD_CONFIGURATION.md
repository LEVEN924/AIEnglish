...
- 用户开通新版口语评测后，ASR / SOE-N 连续三轮真实调用均成功。ASR 使用 `16k_en`，538–766 ms；短音频 SOE-N 为 8540–8578 ms，第三轮额外校验了声学评分、模型、各评分字段和返回转写。
...
- 通过实际公网 `https://cayi-ai.top:20057`，已完成一段 **64.296 秒、2,057,550 字节 WAV** 的上传 → 腾讯评分 → 读取上次录音检查；JSON 请求 2,743,504 字节，评分请求约 37.1 秒，回听下载约 93 ms，下载内容与上传原音频逐字节相同。
...
| 长录音已上传但页面先报超时 | 评分按音频流处理，需要足够响应时间；当前客户端评分 420 秒、Nginx 上游读取 360 秒。普通接口仍保留原超时，评分不自动重复提交 |
