...
> 文档版本：v1.0<br>
> 创建日期：2026-08-18<br>
> 当前阶段：1000 篇 SQLite 数据库版 1.0 已实现并运行<br>
...
- [x] 完成跳过当前文章并切换下一篇内容。
- [x] 完成内容结构、质量、去重和来源检查流水线；1000/1000 篇内容通过，来源 URL 唯一且可追溯。
...
- [x] 将内容库扩充到 1000 篇并写入 SQLite；L1/L2/L3 配额为 350/400/250。
- [x] 接入系统 TTS、逐句定位、翻译/写作量表批改和基于口述文本的口语评分；真实声学发音评测待接入。
- [x] 建立 SQLite 数据库以及电脑、手机共享的进度、作答、评分、错题和复习同步。
- [x] 完成提交版本、低分重做、错题生成、标记掌握和每日总结持久化闭环。
- [x] 完成 PWA 清单、离线应用壳和生产模式一键启停。
- [x] 完成桌面 1440×1000 与手机 390×844 自动化 UI 验收，无横向溢出或控制台错误。
...
- [x] 语音统一接入腾讯云：TTS 负责文章与重点词朗读，ASR 作为可替换转写适配层，SOE-N 对真实录音进行成人严格声学评测；不接入 DeepSeek，不使用本地口语评分。
